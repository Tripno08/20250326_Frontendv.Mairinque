...
describe('InterventionExample', () => {
  ...
  it('renderiza a galeria de intervenções', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Treinamento de Habilidades Sociais' })
    ).toBeInTheDocument();
  ...
  it('filtra as intervenções corretamente', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByRole('heading', { name: 'Treinamento de Habilidades Sociais' })
      ).not.toBeInTheDocument();
    });
  });
  ...
  it('filtra por busca de texto', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByRole('heading', { name: 'Regulação Emocional' })
      ).not.toBeInTheDocument();
    });
  });
});
