  ...
  const filteredInterventions = useMemo(() => {
    return interventions.filter(intervention => {
      ...
      const matchesEvidence =
        !filters.evidenceLevel || intervention.evidenceLevel === filters.evidenceLevel;
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm space-y-4">
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex flex-col">
            <label htmlFor="evidence-select" className="sr-only">
              Todos os Níveis de Evidência
            </label>
            <select
              id="evidence-select"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <AnimatePresence>
          {filteredInterventions.map(intervention => (
            ...
            >
              <InterventionCard intervention={intervention} onClick={onInterventionSelect} />
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
