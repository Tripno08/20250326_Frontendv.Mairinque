...
export default function FirstAccessPage() {
  ...
  // Pegar token da URL para preenchimento automático
  const searchParams = useSearchParams();
  const tokenFromQuery = searchParams.get('token') || undefined;

  // Função para lidar com a solicitação de primeiro acesso
  const handleFirstAccess = async (data: FirstAccessRequest) => {
    setLoading(true);
    setError(undefined);
    ...
    try {
      ...
      // Validações adicionais do servidor (simuladas)
      if (data.token !== '12345678' && !tokenFromQuery) {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Não foi possível completar seu primeiro acesso. Por favor, tente novamente.'
      );
    } finally {
      setLoading(false);
    }
